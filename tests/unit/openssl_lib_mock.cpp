...
int OpenSSLLib::SSL_sk_X509_num(STACK_OF(X509) * stack) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_sk_X509_num(stack);
}

X509 *OpenSSLLib::SSL_sk_X509_value(STACK_OF(X509) * stack, int idx) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_sk_X509_value(stack, idx);
}

X509 *OpenSSLLib::SSL_sk_X509_shift(STACK_OF(X509) * stack) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_sk_X509_shift(stack);
}

void OpenSSLLib::SSL_sk_X509_pop_free(STACK_OF(X509) * stack) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_sk_X509_pop_free(stack);
}
...
{
    return OpenSSLLibMockManager::getMockInterface().SSL_X509_digest(data, type, md, len);
}
int OpenSSLLib::SSL_X509_pubkey_digest(const X509 *data,
                                       ...
                                       unsigned int *len) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_X509_pubkey_digest(data, type, md, len);
}
...
void OpenSSLLib::SSL_EVP_MAC_CTX_free(EVP_MAC_CTX *ctx) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_CTX_free(ctx);
}

EVP_MAC_CTX *OpenSSLLib::SSL_EVP_MAC_CTX_new(EVP_MAC *mac) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_CTX_new(mac);
}

int OpenSSLLib::SSL_EVP_MAC_final(EVP_MAC_CTX *ctx,
                                  unsigned char *out,
                                  size_t *outl,
                                  size_t outsize) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_final(ctx, out, outl, outsize);
}

int OpenSSLLib::SSL_EVP_MAC_update(EVP_MAC_CTX *ctx,
                                   const unsigned char *data,
                                   int datalen) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_update(ctx, data, datalen);
}

int OpenSSLLib::SSL_EVP_MAC_init(EVP_MAC_CTX *ctx,
                                 const unsigned char *key,
                                 int keylen,
                                 const OSSL_PARAM params[]) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_init(ctx, key, keylen, params);
}

EVP_MAC *OpenSSLLib::SSL_EVP_MAC_fetch(OSSL_LIB_CTX *libctx,
                                       const char *algorithm,
                                       const char *properties) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_fetch(
            libctx, algorithm, properties);
}

void OpenSSLLib::SSL_EVP_MAC_free(EVP_MAC *mac) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_EVP_MAC_free(mac);
}

OSSL_PARAM OpenSSLLib::SSL_OSSL_PARAM_construct_utf8_string(const char *key,
                                                            char *buf,
                                                            size_t bsize) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_OSSL_PARAM_construct_utf8_string(
            key, buf, bsize);
}
...
{
    return OpenSSLLibMockManager::getMockInterface().SSL_PKCS12_create(
...
{
    return OpenSSLLibMockManager::getMockInterface().SSL_PKCS12_set_mac(
            pkcs12, pass, passlen, salt, saltlen, iter, md_type);
}

void OpenSSLLib::SSL_PKCS12_free(PKCS12 *pkcs12) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_PKCS12_free(pkcs12);
}
int OpenSSLLib::SSL_PKCS12_parse(
        PKCS12 *p12, const char *pass, EVP_PKEY **pkey, X509 **cert, STACK_OF(X509) * *ca) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_PKCS12_parse(p12, pass, pkey, cert, ca);
}
int OpenSSLLib::SSL_i2d_PKCS12_bio(BIO *bp, PKCS12 *pkcs12) noexcept
{
    return OpenSSLLibMockManager::getMockInterface().SSL_i2d_PKCS12_bio(bp, pkcs12);
}
